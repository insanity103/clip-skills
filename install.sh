#!/usr/bin/env bash
# Install the nine clip skills for Claude Code (Linux or macOS), set up their Python environment, then self-test.
...
SKILLS=(gameplay-clip-cutter vertical-clip-renderer clip-overlay-builder clip-audio-mixer clip-cover-picker clip-stylizer clip-subject-mask clip-subject-tracker clipping-campaign-producer)
...
"$VENV/bin/python" -m pip install --quiet --disable-pip-version-check --upgrade pillow numpy
