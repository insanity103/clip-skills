...
- **Triaging Call of Duty multiplayer footage:** read `references/medal_signal.md` — the game's own on-screen medal banners (HEADSHOT, REVENGE, LONGSHOT, etc.) are a second, audio-independent signal for real action, useful for cross-checking a loudness-scored candidate; never build a hook or overlay around one.
